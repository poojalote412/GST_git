...
//            echo $data;
    ...
    public function get_file_loacation() {
        ...
        $data = "";
//        $result_firm_name_dd = $this->db->query("SELECT customer_name,customer_email_id,customer_contact_number,created_on FROM customer_header_all WHERE `firm_id`='$firm_id'");
        $query = $this->db->query("select id,file_location,created_on from observation_transaction_all where customer_id='cust_1010' and insert_id='insert_1001' and file_location!="" and file_location !='1'");
         if ($query->num_rows() > 0) {
            $result = $query->result();

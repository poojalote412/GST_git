...
//            $count = count($month);
            ...
            for ($m = 0; $m < count($month); $m++) {
                ...
                $quer3B_offset = $this->db->query("insert into 3b_offset_summary_all (`month`,`outward_liability`,`rcb_liablity`,`ineligible_itc`,`net_itc`,`paid_in_credit`,`paid_in_cash`,`interest_late_fee`,`late_fees`,`due_date`,`filling_date`)"
                        . " values ('$month[$m]','$liability_on_outward[$m]','$rcm_liability[$m]','$itc_ineligible[$m]','$net_rtc[$m]','$paid_in_credit[$m]','$paid_in_cash[$m]','$interest_late_fees[$m]','$late_fees[$m]','$due_date[$m]','$filling_date[$m]')");
                if ($this->db->affected_rows() > 0) {
                    $values++;
                }
            }
